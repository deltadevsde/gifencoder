...
GIFEncoder.prototype.endBufferedFrames = function(){
  var self = this;
  this.bufferedFrames.forEach(function(frame){
    self.indexedPixels = frame;
    if (self.firstFrame) {
      self.writeLSD(); // logical screen descriptor
    ...
    if (!self.firstFrame) self.writePalette(); // local color table
...
GIFEncoder.prototype.addFrameForGlobalPalette = function(imageData) {
  // HTML Canvas 2D Context Passed In
  if (imageData && imageData.getImageData) {
    this.image = imageData.getImageData(0, 0, this.width, this.height).data;
  } else {
  ...
  this.getImagePixels(); // convert to correct format if necessary
  this.analyzePixels(); // build color table & map pixels
  this.bufferedFrames.push(this.indexedPixels.map(function(a){return a;}));
};
...
GIFEncoder.prototype.analyzePixels = function() {
  ...
  var tempTable = imgq.getColormap();
  for(i = 0; i < this.colorTab.length; i+=3){
    cname = this.colorTab[i] << 16 | this.colorTab[i+1] << 8 | this.colorTab[i+2];
    this.colorIndex[cname] = [this.colorTab[i],this.colorTab[i+1],this.colorTab[i+2]] ;
  }
  for(i = 0; i < this.tempTable.length; i+=3){
    cname = tempTable[i] << 16 | tempTable[i+1] << 8 | tempTable[i+2];
    if(this.colorIndex[cname] == null){
      this.colorIndex[cname] = [tempTable[i],tempTable[i+1],tempTable[i+2]];
      this.colorTab.push(tempTable[i],tempTable[i+1],tempTable[i+2]);
    }
  }
  ...
  for (var j = 0; j < nPix; j++) {
    var index = this.findClosest(
        (this.pixels[k++] & 0xff)<<16 |
        (this.pixels[k++] & 0xff )<<8 |
        (this.pixels[k++] & 0xff)
  );
