...
GIFEncoder.prototype.analyzePixels = function() {
  ...
  if(this.newQuant == null) {
    this.newQuant = new NeuQuant(this.pixels, this.sample);
    this.newQuant.buildColormap(); // create reduced palette
  }
  this.colorTab = this.newQuant.getColormap();

  // map image pixels to new palette
  var k = 0;
  for (var j = 0; j < nPix; j++) {
    var index = this.newQuant.lookupRGB(
      this.pixels[k++] & 0xff,
      this.pixels[k++] & 0xff,
      this.pixels[k++] & 0xff
    );
